...
const DEV_PORT_ENV_KEYS = ['WORKMESH_DEV_PORT', 'DEV_SERVER_PORT', 'PORT'] as const;
const LOGGER_PORT_ENV_KEYS = ['WORKMESH_LOGGER_PORT', 'DEV_LOGGER_PORT', 'LOGGER_PORT'] as const;
...
const apkName = 'Workmesh_' + packageJson.version + '_' + (process.env.arch || process.arch);
...
module.exports = {
  packagerConfig: {
    ...
    tmpdir: path.resolve(__dirname, '../Workmesh-tmp'),
    extraResource: [path.resolve(__dirname, 'public')],
    win32metadata: {
      CompanyName: 'workmesh',
      FileDescription: 'AI Agent Desktop Interface',
      OriginalFilename: 'Workmesh.exe', // 简化文件名
      ProductName: 'Workmesh',
      InternalName: 'Workmesh',
  ...
  makers: [
    ...
    ...(MakerSquirrel
      ? [
          new MakerSquirrel(
            {
              name: 'Workmesh', // 必须与 package.json 的 name 一致
              authors: 'workmesh', // 任意名称
